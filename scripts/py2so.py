'''
@Author       : gongzhang4
@Date         : 2026-01-07 09:47:07
@LastEditors  : zhanggong1 zhanggong1@sungrowpower.com
@LastEditTime : 2026-01-07 09:47:08
@FilePath     : py2so.py
@Description  :
'''
...
def getpy(
    basepath=os.path.abspath('.'), parentpath='', name='', build_dir="build", excepts=(), copyOther=False, delC=False
):
    """
    获取py文件的路径
    :param basepath: 根路径
...
if __name__ == "__main__":
    ...
    # 需要排除的文件列表（启动文件必须排除）
    exclude_files = [setupfile, os.path.join(currdir, "run.py")]  # 排除自身  # 排除启动文件
    ...
    try:
        # 获取py列表，传入排除文件
        module_list = list(
            getpy(basepath=currdir, parentpath=parentpath, build_dir=build_dir, excepts=tuple(exclude_files))
        )
        print(f"待编译文件: {module_list}")

        if not module_list:
            print("警告: 没有找到需要编译的Python文件！")
            sys.exit(0)
        ...
        print("cleaning...")
        module_list = list(
            getpy(basepath=currdir, parentpath=parentpath, build_dir=build_dir, excepts=tuple(exclude_files), delC=True)
        )
