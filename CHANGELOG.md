...
- panel-label 生产服务同时装载 MVS 插件，两个场景共用 3001 端口；镜像构建、
  原子热更新和离线发布同步包含 MVS wheel、entry point 与五组模型权重。

### 框架架构

- 将日志测试配置移入 `test/conftest.py`，取消插件测试对根目录测试配置的隐式依赖。
- 合并单图与批量请求的参数日志调用及输出配置，整理摘要与 HTTP 生命周期代码，保持日志字段和行为不变。
- 请求日志贯穿请求编号、场景、型号与文件名，统一关键参数摘要及判定结果，
  移除截断原文与完整对象的重复输出；异常摘要单行记录，推理错误保留异常链。
- HTTP 日志补充接收字节、接收等待、进入业务前、响应发送和后台任务耗时；
  流式透传请求体，不预读或复制上传内容。
- 回流记录增加请求编号与图片保存状态，记录 JSON 保存完成或失败事件；
  pytest 日志使用独立临时目录，避免测试异常混入运行日志。
- 单图路由增加可选的原始文件名输入构建钩子，供按业务文件名决定检测步骤的场景
  复用统一上传、响应、统计与回流生命周期。
- 调用统计与场景自定义回流目录解耦，`unmatch` 等失败子分类按标准 `ng` 计数，
  避免自定义目录名导致统计写入失败。
- RF-DETR 分割后处理支持配置 mask 概率阈值，并在 logit 空间完成二值化。
- RF-DETR 公共推理器新增两输出纯检测模型支持，同时保持三输出分割模型契约不变。
- 数据回流目标支持安全的型号子目录，供场景按物料版本分层归档。
- 指示灯中间结果补充原图宽高，为插件持久化注册灯位布局和几何匹配提供明确坐标基准。
...
- 线标场景支持通过 `line_order` 分号分隔多组候选顺序，任一候选匹配即可通过；
  Swagger 示例同步展示多候选请求格式。

### 框架架构

- 新增公共坐标区域、检测结果、OCR Token 和 PASS/FAIL/REVIEW 判定类型，统一场景
  中间结果的数据含义；旧 `status` 字段继续保留，响应新增明确的 `verdict` 字段。
- 抽取公共视觉参考参数与 AICameraModel 请求结构，供直流熔丝、指示灯、线路压缩
  和线标等场景复用，避免场景间重复定义同一外部契约。
- 数据回流、可视化和调用统计支持 REVIEW 独立分类，避免复核样本被归入 NG。
- 新增公共 `SceneSettings` 场景配置基类，统一 `.env`、环境变量覆盖和类型校验；
  非法配置在启动阶段明确失败，不再由场景手写解析并静默回退。

## [2.2.2] - 2026-07-28（Git tag: `v2.2.2`）

### 变更

- **代码结构简化**：收敛单图与批量路由的结果转换，简化路由发现、回流扩展名解析
  及发布脚本结构；测试权重改用临时夹具，避免依赖本地部署模型。

### 框架架构

- **批量场景组合能力**：新增批量路由与业务基类，统一复用上传暂存、推理准入、
  调用统计和逐图关联回流，保持既有单图片链路不变。
- **多模型生命周期**：新增按角色管理的 Runner Group，统一初始化失败回滚和逆序
  幂等释放；运行状态可选展示场景与模型角色，区分同名 ONNX 模型。
- **MVS 场景接入**：登记 `vie-plugin-mvs` 子模块，提供装箱清单与实物标签的多图片
  物料检验能力；生产镜像、Compose 和发布脚本保持不变。

---

## [2.2.1] - 2026-07-23（Git tag: `v2.2.1`）

### 修复
- **Panel 测试部署隔离**：panel-label 测试容器使用独立的
  `mobile-vision-panel-label-test` 名称和宿主机 3004 端口，避免影响 3001 旧生产服务。
- **热更新权重清单兼容**：远程激活忽略权重清单中的空行，修复 `conda run`
  追加尾部换行时误报空权重路径、阻止版本激活的问题。
- **基础镜像标签修复**：运行时 `base` 阶段重新声明 CUDA 构建参数，确保
  `io.vie.base-image` 写入实际镜像地址并通过发布合同校验。
