...
@LastEditTime : 2026-01-19 08:59:55
...
class RouterRegistry:
    ...
    def find_routers(self, package_name: str) -> List[Tuple[str, APIRouter]]:
        """
        查找指定包下的所有路由模块并返回其路由实例

        :param package_name: 包名，如 'routers'
        :return: 包含 (模块名, 路由实例) 元组的列表
        """
        routers = []
        try:
        ...
        for importer, module_name, ispkg in pkgutil.iter_modules(package_path):
            if module_name == 'router_registry' or module_name == 'base_router':
                continue
            try:
    ...
    def register_all_routers(self, app: FastAPI, package_name: str) -> None:
        """
        注册所有路由到FastAPI应用

        :param app: FastAPI应用实例
        :param routers: 包含 (模块名, 路由实例, 配置) 元组的列表
        """
